...
async def account_login(bot: Client, m: Message):
    editable = await m.reply_text(f"Hi 👋 ͢🦋⃟≛⃝𝗘𝗶𝗻𝘀𝘁𝗲𝗶𝗻_𝗔𝗺𝗮𝗿𝗷𝗲𝗲𝘁⃝❤ ! How are You?\n\n☞ I'm a High Speed **Txt File** Downloader Bot.\n\n☞ I can Download **Videos & Pdf** From Your **TXT** File send command /txt or download vision ias command /vpdf.\n\n**𝐃𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐝 𝐁𝐲 :** ͢🦋⃟≛⃝𝗘𝗶𝗻𝘀𝘁𝗲𝗶𝗻_𝗔𝗺𝗮𝗿𝗷𝗲𝗲𝘁⃝❤")
...
async def account_login(bot: Client, m: Message):
    ...
    await editable.edit("**Enter Resolution Ex :** 360,480,720 Or 1080")
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:
                cc = f'[ 🎬 ] **Vid ID : **{str(count).zfill(3)}\n**Video Title :** {name1}({res}) 『 Einstein Amarjeet』.mp4\n**Batch Name :** {b_name}\n\n**Extracted By ➤** {CR}'
                cc1 = f'[ 📕 ] **Pdf ID : **{str(count).zfill(3)}\n**File Title :** {name1} 『 Einstein Amarjeet 』.pdf\n**Batch Name :**{b_name}\n\n**Extracted By ➤** {CR}'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1002060191058)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
...
async def vision_pdf(bot: Client, m: Message):
    ...
    try:
        for i in range(count, len(links)):
            ...
            cc = f'{str(count).zfill(3)}. {name1}.pdf\n\n**Batch:-** {raw_texty}\n\n**Extracted By ➤** ͢🦋⃟≛⃝𝗘𝗶𝗻𝘀𝘁𝗲𝗶𝗻_𝗔𝗺𝗮𝗿𝗷𝗲𝗲𝘁⃝❤'
        ...
        await m.reply_text(e)
    await m.reply_text("Done Ho Gya  ͢🦋⃟≛⃝𝗘𝗶𝗻𝘀𝘁𝗲𝗶𝗻_𝗔𝗺𝗮𝗿𝗷𝗲𝗲𝘁⃝❤ Sir✅")
